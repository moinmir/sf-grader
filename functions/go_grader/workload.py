...
def app_handle(args, state, syscall):
    ...
    with tempfile.NamedTemporaryFile(suffix=".tar.gz") as submission_tar:
        submission_tar_data = syscall.read_key(
            bytes(args["submission"], "utf-8"))
        ...
        with tempfile.TemporaryDirectory() as submission_dir:
            ...
            os.system("tar -C %s -xzf %s --strip-components=1" %
                      (submission_dir, submission_tar.name))
            ...
            with tempfile.NamedTemporaryFile(suffix=".tar.gz") as script_tar:
                ...
                with tempfile.TemporaryDirectory() as script_dir:
                    os.system("tar -C %s -xzf %s" %
                              (script_dir, script_tar.name))
                    ...
                    os.putenv("PATH", "%s:%s" %
                              ("/srv/usr/lib/go/bin", os.getenv("PATH")))
                    os.chdir(script_dir)
